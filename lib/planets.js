...
function generatePlanetOutside(planet, speedFactor = 1) {
  ...
  return { position, scale, animation }; // + rotation
}

function generatePlanetInside(planet, speedFactor = 1) {
  const { rotationTime, distance, distanceFromSun,  modelScale } = planetsData[planet];
  ...
  const rotation = { x: 0, y: 0, z: 0 }; // TEST: was 40
  const rotationEnd = planet === "venus" ? -360 : 360;

  const animation = {
    property: "rotation",
    dur: (rotationTime / speedFactor) * 1000,
    to: { x: 0, y: rotationEnd, z: 0 }, // z= 0 TEST: was 40
    loop: true,
    easing: "linear",
  };
  const model = `#${planet}`;
