...
const earthDistance = 1200; // in 3d meters for z ; orig - 200
...
const planetsData = {
  ...
  mercury: {
    ...
    distanceFromSun: 57.9 * distanceFactor, // mmillions of kilometers
    orbitTime: earthYear * 0.24,
    rotationTime: earthDay * 59,
    modelScale: 0.5,
    tilt:  0  // rotation of vertical axis
  },
  venus: {
    distance: earthDistance * 0.72,
    distanceFromSun : 108.2 * distanceFactor,
  ...
  earth: {
    ...
    distanceFromSun: 149.6 * distanceFactor,
    orbitTime: earthYear,
  ...
  mars: {
    ...
    distanceFromSun: 228 * distanceFactor,
  ...
  jupiter: {
    ...
    distanceFromSun: 778.5 * distanceFactor,
  ...
  saturn: {
    ...
    distanceFromSun: 1432 * distanceFactor,
...
function generatePlanetOutside(planet, speedFactor = 1) {
  ...
  // tryout: tilt   -- this here would give tilt for the whole orbit.
  ...
  return { position, scale, animation, rotation }; // + rotation
}

function generatePlanetInside(planet, speedFactor = 1) {
  const { rotationTime, distance, distanceFromSun,  modelScale, tilt } = planetsData[planet];
  const position = { x: 0, y: 0, z: 0 - distanceFromSun };

  const baseScale = 0.4; // was 0.2
  const scaleValue = baseScale * modelScale;
  const scale = { x: scaleValue, y: scaleValue, z: scaleValue };

  const rotation = { x: 0, y: 0, z: 0 }; // tryout: x: tilt
  const rotationEnd = planet === "venus" ? -360 : 360;

  const animation = {
    property: "rotation",
    dur: (rotationTime / speedFactor) * 1000,
    to: { x:0,  y: rotationEnd, z: 0 }, // x= 0 TEST: was 40
    loop: true,
    easing: "linear",
  };
  const model = `#${planet}`;
